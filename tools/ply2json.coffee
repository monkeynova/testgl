...
ply_parse = (buffer,cb) ->
  ...
  header_parsed = ply_parse_header header_string

  body_parsed = ply_parse_body header_parsed, body_buffer

  return body_parsed

ply_parse_header = (header_string) ->
...
ply_parse_body = (header,body_buffer) ->
  reader = { offset: 0, buffer : body_buffer }

  if header.format == "binary_big_endian"
    return ply_parse_body_binary_big_endian header, reader
  if header.format == "ascii"
    return ply_parse_body_ascii header, reader

ply_parse_body_ascii = (header,reader) ->
...
ply_parse_body_binary_big_endian = (header,reader) ->
  body = {}
